...
export const Forms = () => {
    ...
    const editOptions = (fieldIndex) => {
        const newOptions = prompt("Enter new options separated by commas", formFields[fieldIndex].options.join(", "));
        if (newOptions) {
                    ...
                    formFields.map((field, index) => (
                        <div key={index} className="mb-4">
                            <div className="flex justify-between">
                                ...
                                <div>
                                    <button className="px-2 bg-blue-300 rounded-md" onClick={() => changeLable(field.label)}>Edit Label</button>
                            ...
                            {field.type === 'textarea' ? (
                                <textarea className="outline-none border border-gray-300 rounded-md p-2 w-full" rows="3"></textarea>
                            ) : field.type === 'checkbox' ? (
                                <div>
                                    {field.options.map((option, optIndex) => (
                                        <label key={optIndex} className="inline-flex items-center space-x-3">
                                            <input className="form-checkbox h-5 w-5 text-blue-600 bg-gray-200 border-gray-300 rounded focus:ring-blue-500" type="checkbox" />
                                ...
                                <div>
                                    {field.options.map((option, optIndex) => (
                                        <label key={optIndex} className="inline-flex items-center space-x-3">
                                            <input className="form-radio h-5 w-5 text-blue-600 bg-gray-200 border-gray-300 rounded focus:ring-blue-500" type="radio" name={`radioGroup-${index}`} />
                                            <span className="text-gray-700">{option}</span>
                                        </label>
                                    ))}
                                </div>
                            ) : (
                                <input className="outline-none border border-gray-300 rounded-md p-2 w-full" type={field.type} />
                            )}
                        </div>
                    ))
                ) : (
